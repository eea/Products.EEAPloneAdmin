...
translateFromSite = (('address.html', 'Document', None),
                     ...
                     ('pressroom/newsreleases/all-press-releases', 'Topic',
                      'News releases'),
...
def getLanguages(context):
    ...
    if DevelopmentMode:
        logger.info("EEAPloneAdmin:local-sites: we are in DEBUG we only "
                    "run for %s languages" % EEA_LANGUAGES)
...
def fixPromotion(context):
    ...
    for lang, _unused in languages:
        promotion = qlinks.educational[
            'glossary-learn-about-environmental-terms']
    ...
    for obj in qlinks.educational.objectValues():
        for lang, _unused in languages:
            ...
            if translation is not None:
                if (wf.getInfoFor(translation, 'review_state') != 'published'
                    and wf.getInfoFor(obj, 'review_state') == 'published'):
                    wf.doActionFor(translation, 'publish', comment=(
    ...
    for b in catalog(portal_type=['PressRelease', 'Highlight', 'Promotion'],
        ...
        for _lang, t in canonical.getTranslations().items():
            ...
            for field in independent_fields:
                ...
                data = accessor()
                translation_mutator = getattr(translation,
                                              field.translation_mutator)
                translation_mutator(data)
            translation.reindexObject()
    logger.info("Copied language independent fields for PressRelease,Highlight,"
                " Promotion ")
