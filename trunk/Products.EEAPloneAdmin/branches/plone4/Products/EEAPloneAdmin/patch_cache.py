...
def PatcheddoNotCache(published, request, response):
...
plone.app.caching.operations.utils.doNotCache = PatcheddoNotCache

def PatchedcacheInBrowser(published, request, response, etag=None, lastModified=None):
    """ Added extra `` no-store``, ``no-cache``, ``post-check``,
        ``pre-check`` and ``Pragma`` on headers """
...
plone.app.caching.operations.utils.cacheInBrowser = PatchedcacheInBrowser
