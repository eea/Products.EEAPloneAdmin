...
def QuickUploadCapableFileFactory__call__(self, name, title, description, content_type, data, portal_type):
    context = aq_inner(self.context)
                ...
                if obj :
                    ...
                    if primaryField is not None:
                        ...
                        if not obj.getFilename() :
                            ...
                            #patch:
                            #check if setFilename worked, if not we set the filename on the blob
                            if not obj.getFilename():
